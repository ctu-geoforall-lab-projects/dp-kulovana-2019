...
%% ML: sudo -> administratora (citelnejsi pro uzivatele, ktery nezna Linux)
\textit{sudo}. Některé z nich umožňují použití parametrů, pro část
%% ML: ta druha cast vety neni moc srozumitelna 
...
%% ML: nastrel -> neco vhodnejsiho, navrh...
navázat na nástřel existující knihovny pro správu uživatelů z roku
...
%ldap_auth.py
%% ML: se sestává (to slovo mas v praci naduzivano, je evidetne Tvoje oblibene ;-)
...
%% ML: v tomto ohledu se web konzole chova jinak nez admin prikazy
%% (maze pouze prazdne skupiny). nemusis menit, pouze znimit
...
%% ML: se sestava (opakujici se slovo)
