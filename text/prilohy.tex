...
\item{Go to the project directory (e.g. \textsf{/mnt/home/gislab/web\_console}), open file \textsf{web\_console\_project/settings\_custom.py} and assign the variable \linebreak\textsf{ALLOWED\_HOSTS} to your GIS.lab server \zk{URL}.}
...
\item{Sync your existing \zk{LDAP} users and groups to Django database by running:}
\begin{center}
...
\item{Go to the homepage at \textsf{ALLOWED\_HOSTS:8080} where \textsf{ALLOWED\_HOSTS} stands for assigned \zk{URL}.}
...
\textit{Username, first name, last name, email address} and \textit{password} are mandatory fields for successful registration, \textit{description} is optional. For sign up push the \textsf{SIGN UP} button. If the form is not valid, the error, with information of what went wrong, shows. In case of a successful validation, you are redirected to login page.
\begin{figure}[H] \centering
...
For authenticated user, homepage displays their personal details and active roles. \textsf{Edit} button allows you to change your personal details.
\begin{figure}[H] \centering
...
From all of your atributes, you are not allowed to change \textsf{username} field. For changing password, you need to click on \textsf{this form} link in the text and you will be redirected to the correct page.
\begin{figure}[H] \centering
...
Logout is available by \textsf{LOGOUT} button in the upper right corner.

\section{Admin console}
\label{admin-console}

To access the admin console, you need to write \textit{/admin} behind your homepage \zk{URL} (e.g. http://b802-01.fsv.cvut.cz:8080/admin). Fill in your credentials to log in to the admin console.
\begin{figure}[H] \centering
...
Write new password twice for verification and push the \textsf{CHANGE} button. In case of a successful change, user's credentials are updated.
\begin{figure}[H] \centering
...
On group details page you can delete group from database by pushing \textsf{Delete} button in the bottom left corner. The decision has to be confirmed.
\begin{figure}[H] \centering
